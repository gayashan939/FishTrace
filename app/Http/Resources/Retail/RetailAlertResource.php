...
class RetailAlertResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            'batch' => $this->when($alert->relationLoaded('batch'), fn () => $alert->batch?->only(['id', 'batch_code'])),
        ];
    }
    ...
    private function model(): ColdChainAlert
    {
        if (! $this->resource instanceof ColdChainAlert) {
            throw new LogicException('RetailAlertResource requires a ColdChainAlert model.');
        }
