...
class PackageLabelController extends Controller
{
    ...
    public function print(PackageLabel $label, ProcessingOperationsQuery $operations): Response
    {
        $this->authorize('view', $label);
        $label = $operations->label($label);

        return response()->view('processor.package-label', ['label' => $label, 'batch' => $label->batch]);
    }
}
