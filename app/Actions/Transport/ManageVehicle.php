...
class ManageVehicle
{
    ...
    public function create(User $user, array $data): Vehicle
    {
        ...
        $this->audit->record('VEHICLE_CREATED', $vehicle, null, $vehicle->only(['registration_number', 'name', 'is_active']), $user, $organization->id);
    ...
    public function update(User $user, Vehicle $vehicle, array $data): Vehicle
    {
        ...
        $old = $vehicle->only(['registration_number', 'name', 'is_active']);
        $vehicle->update($data);
        $this->audit->record('VEHICLE_UPDATED', $vehicle, $old, $vehicle->only(['registration_number', 'name', 'is_active']), $user, $vehicle->organization_id);
    ...
    public function delete(User $user, Vehicle $vehicle): void
    {
        ...
        $this->audit->record('VEHICLE_DELETED', $vehicle, $vehicle->only(['registration_number', 'name', 'is_active']), null, $user, $vehicle->organization_id);
        $vehicle->delete();
    }
}
