<x-layouts.admin title="Catch details | FishTrace">
    @include('admin.fishing._nav')
    <a href="{{ route('admin.fishing.catches.index') }}" class="text-sm text-cyan-700">← Catches</a><h1 class="mt-2 text-3xl font-bold">Catch details</h1><p class="text-slate-500">{{ $catch->species?->common_name }} · {{ $catch->caught_at?->format('Y-m-d H:i:s') }}</p>
